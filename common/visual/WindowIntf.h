...
public:
	...
	void InitDrawContext(iTJSDispatch2 *tjs_obj);
...
protected:
	tTJSVariant LayerEventTargetObject;
	tTVPLayerTreeOwner * LayerEventTarget;
public:
	void SetLayerEventTargetObject(const tTJSVariant & val);
	const tTJSVariant & GetLayerEventTargetObject() const { return LayerEventTargetObject; }
	tTVPLayerTreeOwner *GetLayerEventTarget() const { return LayerEventTarget;}
...
protected:
	tTVPRect DestRect;
public:
	void SetDestRectangle(tTVPRect &destRect);
	tTVPRect &GetDestRect() { return DestRect; }
...
public:
	virtual void NotifyUpdateRegionFixed(const tTVPComplexRect &updaterects); // is called by layer manager
	virtual void UpdateContent() = 0; // is called from event dispatcher
...
protected:
	bool WaitVSync;
	virtual void UpdateWaitVSync() = 0;
