...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::ChangeManager(tTVPLayerManager *manager)
{
	Update();
	if (Manager) {
	...
	tTVPRect &cliprect)
{
	...
	if(UpdateRectForChild.get_width() < w || UpdateRectForChild.get_height() < h) {
		TVPThrowExceptionMessage(TVPInternalError, TJS_W("Invalid updaterect"));		
	}
...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_CONSTRUCTOR_DECL(/*var.name*/_this, /*var.type*/tTJSNI_BaseLayer,
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/loadImages)
{
	...
	if(numparams < 1) return TJS_E_BADPARAMCOUNT;
	ttstr name(*param[0]);
	tjs_uint32 key = clNone; // TODO Intfなのに固有値
	if(numparams >=2 && param[1]->Type() != tvtVoid)
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/update)
{
	...
	// this event sets CallOnPaint flag of tTJSNI_BaseLayer, to
...
{
	TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_BaseLayer);
	_this->DumpStructure(numparams > 0 ? (tjs_int)*param[0] : 0);
	return TJS_S_OK;
}
...
{
	// Face名をファイル名として開く、FreeTypeでのみ有効。ただし、そのレイヤーでIMEを有効した場合動作DEFAULT_GUI_FONT が使われる
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_Font);
