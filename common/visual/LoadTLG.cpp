	...
	tTVPGraphicLoadMode mode)
{
	...
	TVPReadBuffer(src, mark, 1);
	colors = mark[0];
	width = TVPReadI32LE(src);
	height = TVPReadI32LE(src);
	blockheight = TVPReadI32LE(src);
	...
	TVPStreamSetPosition(src, src->GetPosition() + blockcount * sizeof(tjs_uint32));
	...
	try
	{
		...
		for(tjs_int y_blk = 0; y_blk < height; y_blk += blockheight)
		{
			...
			for(tjs_int c = 0; c < colors; c++)
			{
				TVPReadBuffer(src, mark, 1);
				tjs_int size;
				size = TVPReadI32LE(src);
				if(mark[0] == 0)
				{
					// modified LZSS compressed data
					TVPReadBuffer(src, inbuf, size);
					r = TVPTLG5DecompressSlide(outbuf[c], inbuf, size, text, r);
				}
				else
				{
					// raw data
					TVPReadBuffer(src, outbuf[c], size);
				}
			}
	...
	tTVPGraphicLoadMode mode)
{
	...
	max_bit_length = TVPReadI32LE(src);
	...
	try
	{
		...
		{
			tjs_int inbuf_size = TVPReadI32LE(src);
			tjs_uint8* inbuf = (tjs_uint8*)TJSAlignedAlloc(inbuf_size+16, 4);
			try
			{
				TVPReadBuffer(src, inbuf, inbuf_size);
		...
		for(tjs_int y = 0; y < height; y += TVP_TLG6_H_BLOCK_SIZE)
		{
			...
			{
				// read bit length
				tjs_int bit_length = TVPReadI32LE(src);
				...
				TVPReadBuffer(src, bit_pool, byte_length);
	...
	iTJSBinaryStream *src, tjs_int keyidx,  tTVPGraphicLoadMode mode)
{
	...
	if(!memcmp("TLG0.0\x00sds\x1a\x00", mark, 11))
	{
		...
		// read raw data size
		tjs_uint rawlen = TVPReadI32LE(src);
		...
		while(true)
		{
			...
			if(4 != src->Read(chunkname, 4)) break;
				// cannot read more
			tjs_uint chunksize = TVPReadI32LE(src);
			if(!memcmp(chunkname, "tags", 4))
			{
				...
				try
				{
					...
					TVPReadBuffer(src, tag, chunksize);
					tag[chunksize] = 0;
			...
			{
				// skip the chunk
				TVPStreamSetPosition(src, src->GetPosition() + chunksize);
			}
		} // while
...
//---------------------------------------------------------------------------
void TVPLoadHeaderTLG(void* formatdata, iTJSBinaryStream *src, iTJSDispatch2** dic )
{
	if( dic == NULL ) return;
