...
void TJS_INTF_METHOD tTJSNI_BitmapLayerTreeOwner::NotifyBitmapCompleted(class iTVPLayerManager * manager,
	...
	int _width  = bitmapinfo->GetWidth();
	int _height = bitmapinfo->GetHeight();
	int _pitch  = bitmapinfo->GetPitchBytes(); 
	...
	if( !(x < 0 || y < 0 || x + cliprect.get_width() > w || y + cliprect.get_height() > h) &&
		!(cliprect.left < 0 || cliprect.top < 0 ||
			cliprect.right > _width || cliprect.bottom > _height) )
	{
		tTVPRect destrect(x, y, x+cliprect.get_width(), y+cliprect.get_height());
		if( IsDirty == false ) {
			if( DirtyRectInstance ) {
				DirtyRectInstance->Set( destrect.left, destrect.top, destrect.right, destrect.bottom );
				IsDirty = true;
			}
		} else {
			if( DirtyRectInstance ) {
				TVPUnionRect( &DirtyRectInstance->Get(), DirtyRectInstance->Get(), destrect );
			}
		}
