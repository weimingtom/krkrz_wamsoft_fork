...
{
	TVPReadBuffer(((iTJSBinaryStream *)png_get_io_ptr(png_ptr)), (void*)data, (tjs_uint)length);
}
//---------------------------------------------------------------------------
// read_row_callback
...
{
    iTJSBinaryStream* stream=(iTJSBinaryStream*)png_get_io_ptr(png_ptr);
    TVPWriteBuffer(stream, buf,(tjs_uint)size);
}
//---------------------------------------------------------------------------
/**
 * PNG書き込み
 ...
 */
void TVPSaveAsPNG(void* formatdata, iTJSBinaryStream* dst, const tTVPBaseBitmap* image, const ttstr & mode, iTJSDispatch2* meta )
{
	int bpp = 32;
...
void TVPLoadHeaderPNG(void* formatdata, iTJSBinaryStream *src, iTJSDispatch2** dic )
{
	png_structp png_ptr=NULL;
	png_infop info_ptr=NULL;
	png_infop end_info=NULL;
