...
//---------------------------------------------------------------------------
static void Compress( const tTVPBaseBitmap *bmp, iTJSBinaryStream * out, bool is24 = false )
{
	// compress 'bmp' to 'out'.
	...
	// header
	{
		TVPWriteBuffer(out, "TLG5.0\x00raw\x1a\x00", 11);
	...
	try
	{
		...
		for(int blk_y = 0; blk_y < (int)bmp->GetHeight(); blk_y += BLOCK_HEIGHT, block++)
		{
			...
			for(int c = 0; c < colors; c++)
			{
				...
				if(wrote < inp)
				{
					TVPWriteBuffer(out, "\x00", 1);
					WriteInt32(wrote, out);
					TVPWriteBuffer(out, cmpoutbuf[c], wrote);
					blocksize += wrote + 4 + 1;
				}
				else
				{
					compressor->Restore();
					TVPWriteBuffer(out, "\x01", 1);
					WriteInt32(inp, out);
					TVPWriteBuffer(out, cmpinbuf[c], inp);
					blocksize += inp + 4 + 1;
				}
				written[c] += wrote;
			}
		...
		TVPStreamSetPosition(out, blocksizepos );
		for(int i = 0; i < blockcount; i++)
		{
			WriteInt32(blocksizes[i], out);
		}
		TVPStreamSetPosition(out, pos_save );
...
//---------------------------------------------------------------------------
void SaveTLG5( iTJSBinaryStream* stream, const tTVPBaseBitmap* image, bool is24 )
{
