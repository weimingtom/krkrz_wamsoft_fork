...
//---------------------------------------------------------------------------
GLOBAL(void)
jpeg_TStream_src (j_decompress_ptr cinfo, iTJSBinaryStream * infile)
{
  my_source_mgr * src;
...
METHODDEF(boolean) JPEG_write_empty_output_buffer( j_compress_ptr cinfo ) {
	...
	TVPWriteBuffer(dest->stream, dest->buffer, (tjs_uint)wrotelen );
...
METHODDEF(void) JPEG_write_stream( j_compress_ptr cinfo, JOCTET* buffer, int bufsize, int* outsize, iTJSBinaryStream* stream ) {
 ...
 */
void TVPSaveAsJPG(void* formatdata, iTJSBinaryStream* dst, const tTVPBaseBitmap* image, const ttstr & mode, iTJSDispatch2* meta )
{
	...
	try {
		...
		TVPWriteBuffer(stream, dest_buf, num_write_bytes );
	} catch(...) {
		jpeg_destroy_compress(&cinfo);
		delete[] dest_buf;
		throw;
...
//---------------------------------------------------------------------------
void TVPLoadHeaderJPG(void* formatdata, iTJSBinaryStream *src, iTJSDispatch2** dic )
{
	jpeg_decompress_struct cinfo;
	my_error_mgr jerr;
