...
protected:
	...
	std::shared_ptr<iTJSBinaryStream> File;	 //!< iTJSBinaryStream オブジェクト
...
public:
	tGenericFreeTypeFace(const tjs_string &fontname, tjs_uint32 options);
	tGenericFreeTypeFace(const tjs_string &path, const tjs_string &fontname, tjs_uint32 index );
	tGenericFreeTypeFace(const tjs_string &filename, std::vector<FontInfo*>& fonst, std::vector<tjs_string>* faces );
...
//---------------------------------------------------------------------------
static void TVPLoadFont( FT_Open_Args& arg, std::vector<FontInfo*>& fonts, std::map<FaceKey,FontInfo*>* fontmap, std::vector<tjs_string>* faces, const tjs_string* path, const tjs_string* filename ) {
	...
	for( tjs_uint f = 0; f < face_num; f++ ) {
		...
		if( err == 0 ) {
			...
			std::string face_name = face->family_name;
			if (*face->style_name) {
				face_name += " ";
				face_name += face->style_name;
			}

			tjs_string wname;
			TVPEncodeUTF8ToUTF16( wname, face_name );
 ...
 */
unsigned long IoFunc( FT_Stream stream, unsigned long offset, unsigned char* buffer, unsigned long count )
{
	iTJSBinaryStream * File = static_cast<iTJSBinaryStream*>(stream->descriptor.pointer);
	...
	if(count == 0)
	{
		// seek
		result = 0;
		TVPStreamSetPosition(File, offset );
	}
	else
	{
		// read
		TVPStreamSetPosition(File, offset );
		TVPReadBuffer(File, buffer, count);
		result = count;
	}
...
//---------------------------------------------------------------------------
void tTVPFreeTypeFaceList::LoadSystemFont( tjs_string path, std::vector<tjs_string>* faces ) {
	TVPInitializeFont();
	...
	// ファイルを開く
	std::shared_ptr<iTJSBinaryStream> File = OpenFontFile(path);
	if( File.get() == NULL ) {
	...
	fsr.pos = 0;
	fsr.descriptor.pointer = File.get();
	...
	args.stream = &fsr;
	TVPLoadFont( args, systemfaces_, &systemfont_, faces, &path, &path );
}
//---------------------------------------------------------------------------
...
tBaseFreeTypeFace* tTVPFreeTypeFaceList::GetFace( const tjs_string& facename, tjs_uint32 options ) const {
	if( !faces_.empty() ) {
		...
		if( f != faces_.end() ) {
			FontInfo* font = *f;
			return new tGenericFreeTypeFace( font->filename, TVP_FACE_OPTIONS_FACE_INDEX( font->index ) );
		}
		// スタイル気にせず検索する
	...
	if( i != systemfont_.end() ) {
		...
		return new tGenericFreeTypeFace( font->path, facename, font->index );
	}

	return nullptr;
}

tBaseFreeTypeFace* tTVPFreeTypeFaceList::GetDefaultFace() const {
	if( !systemfaces_.empty() ) {
		// システムに最初に登録されたフェイスを返す
		FontInfo* font = systemfaces_[0];
		return new tGenericFreeTypeFace( font->path, font->facename, font->index );
	}
	return nullptr;
}
 ...
 */
tGenericFreeTypeFace::tGenericFreeTypeFace(const tjs_string &fontname, tjs_uint32 options) : File(NULL)
{
	// フィールドの初期化
	Face = NULL;
...
//---------------------------------------------------------------------------
tGenericFreeTypeFace::tGenericFreeTypeFace(const tjs_string& path, const tjs_string &fontname, tjs_uint32 index ) : File(nullptr), Face(nullptr)
{
	memset(&Stream, 0, sizeof(Stream));
	try {
		// ファイルを開く
		File = OpenFontFile(path);
		if( File.get() == NULL ) {
			TVPThrowExceptionMessage( TVPCannotOpenFontFile, ttstr(path.c_str()) );
		}
 ...
 */
tGenericFreeTypeFace::tGenericFreeTypeFace(const tjs_string &filename, std::vector<FontInfo*>& fonts, std::vector<tjs_string>* faces ) : File(nullptr), Face(nullptr)
{
	// フィールドの初期化
	memset(&Stream, 0, sizeof(Stream));
	...
	try {
		// ファイルを開く
		File = OpenFontFile(filename);
		if( File.get() == NULL ) {
			TVPThrowExceptionMessage( TVPCannotOpenFontFile, ttstr(filename.c_str()) );
		}
...
tFreeTypeFace::tFreeTypeFace(const std::vector<tjs_string> &fontname, tjs_uint32 options)
{
	...
	for( tjs_int i = 0; i < (tjs_int)count; i++ ) {
		...
		if( Faces[i]->Face == nullptr )
		{
			Faces[i]->Face.reset( FreeTypeFaceList->GetDefaultFace() );
		}
