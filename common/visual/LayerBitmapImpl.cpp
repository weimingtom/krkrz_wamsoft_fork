...
tTVPBitmap::tTVPBitmap(tjs_uint width, tjs_uint height, tjs_uint bpp, void* bits)
{
	...
	BitmapInfo = TVPCreateBitmapInfo( width, height, bpp );
	Width = width;
	Height = height;
...
tTVPBitmap::tTVPBitmap(const tTVPBitmap & r)
{
	...
	if (r.BitmapInfo) {
		int width     = r.BitmapInfo->GetWidth();
		int height    = r.BitmapInfo->GetHeight();
		int bpp       = r.BitmapInfo->GetBPP();
		bool unpadding = r.BitmapInfo->GetUnpadding();
		// allocate bitmap which has the same metrics to r
		Allocate(width, height, bpp, unpadding);
		// copy Bits
...
void tTVPBitmap::Allocate(tjs_uint width, tjs_uint height, tjs_uint bpp, bool unpadding)
{
	...
	// create BITMAPINFO
	BitmapInfo = TVPCreateBitmapInfo( width, height, bpp, unpadding );
