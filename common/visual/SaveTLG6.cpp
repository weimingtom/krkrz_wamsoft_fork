...
class TLG6BitStream
{
	...
	iTJSBinaryStream * OutStream; // output stream
	unsigned char *Buffer; // output buffer
	long BufferCapacity; // output buffer capacity

public:
	TLG6BitStream(iTJSBinaryStream * outstream) :
		OutStream(outstream),
		BufferBitPos(0),
		BufferBytePos(0),
		Buffer(NULL),
...
//---------------------------------------------------------------------------
static void WriteInt32(long num, iTJSBinaryStream *out)
{
	char buf[4];
	buf[0] = num & 0xff;
	buf[1] = (num >> 8) & 0xff;
...
//---------------------------------------------------------------------------
// int ftfreq[256] = {0};
void SaveTLG6( iTJSBinaryStream* stream, const tTVPBaseBitmap* bmp, bool is24 )
{
	iTJSBinaryStream *out = stream;
	...
	{
		TVPWriteBuffer(out, "TLG6.0\x00raw\x1a\x00", 11);
		TVPWriteBuffer(out, &colors, 1);
		int n = 0;
		TVPWriteBuffer(out, &n, 1); // data flag (0)
		TVPWriteBuffer(out, &n, 1); // color type (0)
		TVPWriteBuffer(out, &n, 1); // external golomb table (0)
...
//				ftfreq[ft]++;
		...
		// output filter types
		{
			...
			try
			{
				...
				TVPWriteBuffer(out, outbuf, outlen);
			}
			catch(...)
			{
				delete [] outbuf;
		...
		TVPWriteBuffer(out,  memstream->GetInternalBuffer(), (tjs_uint)memstream->GetSize() );
	}
	catch(...)
	{
...
//---------------------------------------------------------------------------
extern void SaveTLG5( iTJSBinaryStream* stream, const tTVPBaseBitmap* image, bool is24 );
//---------------------------------------------------------------------------
void TVPSaveAsTLG(void* formatdata, iTJSBinaryStream* dst, const tTVPBaseBitmap* image, const ttstr & mode, iTJSDispatch2* meta )
{
	...
	try {
		if( tags.size() ) {
			...
			TVPWriteBuffer(stream, "TLG0.0\x00sds\x1a\x00", 11);
			tjs_uint rawlenpos = (tjs_uint)stream->GetPosition();
			TVPWriteBuffer(stream, "0000", 4);
			...
			TVPStreamSetPosition(stream, rawlenpos );
			...
			bin[2] = (size >> 16) & 0xff;
			bin[3] = (size >> 24) & 0xff;
			TVPWriteBuffer(stream, bin, 4);
			TVPStreamSetPosition(stream, pos_save );

			// write "tags" chunk name
			TVPWriteBuffer(stream, "tags", 4);
			...
			// write chunk data
			TVPWriteBuffer(stream,  tagstr.c_str(), (tjs_uint)tagstr.length() );
		} else {
			if( istls6 ) {
				SaveTLG6( stream, image, mode == TJS_W("tlg624") );
			} else {
