...
public:
	tTVPWD_RIFFWave(iTJSBinaryStream *stream, tjs_uint64 datastart,
...
//---------------------------------------------------------------------------
tTVPWD_RIFFWave::tTVPWD_RIFFWave(iTJSBinaryStream *stream, tjs_uint64 datastart,
	const tTVPWaveFormat & format)
{
	Stream = stream;
	DataStart = datastart;
	...
	CurrentPos = 0;
	TVPStreamSetPosition(Stream, DataStart);
}
//---------------------------------------------------------------------------
...
static bool TVPFindRIFFChunk(tTVPStreamHolder & stream, const tjs_uint8 *chunk)
{
	...
	while(true)
	{
		...
		{
			// skip to next chunk
			tjs_uint32 chunksize = TVPReadI32LE(stream.Get());
...
//		bool do_reorder_5poiont1ch = false;
		...
		tjs_uint16 format_tag = TVPReadI16LE(stream.Get()); // wFormatTag
		if(format_tag != WAVE_FORMAT_PCM &&
			format_tag != WAVE_FORMAT_IEEE_FLOAT &&
			format_tag != WAVE_FORMAT_EXTENSIBLE) return NULL;


		format.Channels = TVPReadI16LE(stream.Get()); // nChannels
		format.SamplesPerSec = TVPReadI32LE(stream.Get()); // nSamplesPerSec

		if(4 != stream->Read(buf, 4)) return NULL; // nAvgBytesPerSec; discard

		tjs_uint16 block_align = TVPReadI16LE(stream.Get()); // nBlockAlign

		format.BitsPerSample = TVPReadI16LE(stream.Get()); // wBitsPerSample

		tjs_uint16 ext_size = TVPReadI16LE(stream.Get()); // cbSize
		if(format_tag == WAVE_FORMAT_EXTENSIBLE)
		{
			...
			format.BitsPerSample = TVPReadI16LE(stream.Get()); // wValidBitsPerSample
			format.SpeakerConfig = TVPReadI32LE(stream.Get()); // dwChannelMask
