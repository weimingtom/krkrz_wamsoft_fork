...
//---------------------------------------------------------------------------
static iTJSBinaryStream * TVPGetCachedArchiveHandle(void * pointer, const ttstr & name)
{
...
*/
//---------------------------------------------------------------------------
static bool TVPGetXP3ArchiveOffset(iTJSBinaryStream *st, const ttstr name,
	tjs_uint64 & offset, bool raise)
{
	TVPStreamSetPosition(st, 0);
	tjs_uint8 mark[11+1];
...
tTVPXP3Archive::tTVPXP3Archive(const ttstr & name) : tTVPArchive(name)
{
	...
	iTJSBinaryStream *st = TVPCreateStream(name);
	...
	try
	{
		...
		TVPStreamSetPosition(st, 11 + offset);
		...
		while(true)
		{
			...
			tjs_uint64 index_ofs = TVPReadI64LE(st);
			TVPStreamSetPosition(st, index_ofs + offset);

			// read index to memory
			tjs_uint8 index_flag;
			TVPReadBuffer(st, &index_flag, 1);
			tjs_uint index_size;
			...
			{
				// compressed index
				tjs_uint64 compressed_size = TVPReadI64LE(st);
				tjs_uint64 r_index_size = TVPReadI64LE(st);
				...
				try
				{
					TVPReadBuffer(st, compressed, (tjs_uint)compressed_size);
				...
				TVP_XP3_INDEX_ENCODE_RAW)
			{
				...
				indexdata = new tjs_uint8[index_size];
				TVPReadBuffer(st, indexdata, index_size);
			}
			else
			{
...
//---------------------------------------------------------------------------
iTJSBinaryStream * tTVPXP3Archive::CreateStreamByIndex(tjs_uint idx)
{
	if(idx >= ItemVector.size()) TVPThrowExceptionMessage(TVPReadError);

	tArchiveItem &item = ItemVector[idx];

	iTJSBinaryStream *stream = TVPGetCachedArchiveHandle(this, Name);

	iTJSBinaryStream *out;
	try
	{
...
public:
	...
	void SetData(unsigned long outsize, iTJSBinaryStream *instream,
...
tTVPXP3ArchiveStream::tTVPXP3ArchiveStream(tTVPXP3Archive *owner,
	...
	std::vector<tTVPXP3ArchiveSegment> *segments, iTJSBinaryStream * stream,
		tjs_uint64 orgsize)
{
...
void tTVPXP3ArchiveStream::EnsureSegment()
{
	...
	if(LastOpenedSegmentNum == CurSegmentNum)
	{
		if(!CurSegment->IsCompressed)
			TVPStreamSetPosition(Stream, CurSegment->Start + SegmentPos);
		return;
	}
	...
	if(CurSegment->IsCompressed)
	{
		...
		{
			// too large to cache
			TVPStreamSetPosition(Stream, CurSegment->Start);
...
tjs_uint TJS_INTF_METHOD tTVPXP3ArchiveStream::Read(void *buffer, tjs_uint read_size)
{
	...
	while(read_size)
	{
		...
		else
		{
			...
			TVPReadBuffer(Stream, (tjs_uint8*)buffer + write_size, one_size);
		}
...
void TJS_INTF_METHOD tTVPXP3ArchiveStream::SetEndOfStorage()
{
	...
	try
	{
		...
		try
		{
			...
			for(tjs_int i = 0; i < count; i++)
			{
				...
				iTJSBinaryStream *src = arc->CreateStreamByIndex(i);
				try
				{
					...
					try
					{
						...
						while(true)
						{
							...
							if(read == 0) break;
							TVPWriteBuffer(dest, buffer, read);
						}
					}
					catch(...)
					{
