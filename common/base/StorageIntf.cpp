...
static iTJSBinaryStream * _TVPCreateStream(const ttstr & _name, tjs_uint32 flags)
{
	...
	ttstr name;
#if MY_USE_MINLIB
TVPLOG_DEBUG("_TVPCreateStream: _name:{}", _name);
#endif
	...
	iTJSBinaryStream * result = _InnerTVPCreateStream(name, flags);
	ttstr res = (result?"not null":"null");
	TVPLOG_DEBUG("end _TVPCreateStream: name:{}, result:{}", name, res);
	return result;
}
