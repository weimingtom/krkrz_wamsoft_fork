		...
		try
		{
			...
			else if(mark[0] == 0xfe && mark[1] == 0xfe)
			{
				...
				if(CryptMode == 2)
				{
					...
					tjs_uint64 compressed = TVPReadI64LE(Stream);
					tjs_uint64 uncompressed = TVPReadI64LE(Stream);
					...
					try
					{
						TVPReadBuffer(Stream, nbuf, (unsigned long)compressed);
			...
			else
			{
				...
				if(mark[0] == 0xef && mark[1] == 0xbb && mark2[0] == 0xbf) {
					...
					try
					{
						TVPReadBuffer(Stream, nbuf, size);
...
public:
			...
			if (! CompressionFailed) {
				try {
					...
					do {
						...
						TVPWriteBuffer(Stream, CompressionBuffer, COMPRESSION_BUFFER_SIZE - ZStream->avail_out);
						ZStream->next_out = reinterpret_cast<Bytef*>( CompressionBuffer );
						ZStream->avail_out = COMPRESSION_BUFFER_SIZE;
					} while (result != Z_STREAM_END);

					// rollback and write compression size.
					TVPStreamSetPosition(Stream, CompressionSizePosition);
	...
	void WriteRawData(void *ptr, size_t size)
	{
		if(CryptMode == 2)
		{
			...
			while (ZStream->avail_in > 0) {
				...
				if (ZStream->avail_out == 0) {
					TVPWriteBuffer(Stream, CompressionBuffer, COMPRESSION_BUFFER_SIZE);
					ZStream->next_out = reinterpret_cast<Bytef*>( CompressionBuffer );
					ZStream->avail_out = COMPRESSION_BUFFER_SIZE;
				}
			}
		...
		{
			TVPWriteBuffer(Stream, ptr, (tjs_uint)size); // write directly
		}
	}
