...
//---------------------------------------------------------------------------
bool tTJS::LoadByteCode( class iTJSBinaryStream* stream, tTJSVariant *result,
	iTJSDispatch2 *context, const tjs_char *name )  {
	bool ret = false;
	tjs_uint8* buff = NULL;
	try {
...
//---------------------------------------------------------------------------
bool tTJS::LoadBinaryDictionayArray( class iTJSBinaryStream* stream, tTJSVariant *result )
{
...
//---------------------------------------------------------------------------
void tTJS::CompileScript( const tjs_char *script, class iTJSBinaryStream* output, bool isresultneeded, bool outputdebug, bool isexpression, const tjs_char *name, tjs_int lineofs )
{
...
{
	if(stream->Read(buffer, read_size) != read_size)
		TJS_eTJSError(TJSReadError);
}
//---------------------------------------------------------------------------
void TVPWriteBuffer(iTJSBinaryStream *stream, const void *buffer, tjs_uint write_size)
{
	if(stream->Write(buffer, write_size) != write_size)
		TJS_eTJSError(TJSWriteError);
}
//---------------------------------------------------------------------------
void TVPStreamSetPosition(iTJSBinaryStream *stream, tjs_uint64 pos)
{
	if(pos != stream->Seek(pos, TJS_BS_SEEK_SET))
		TJS_eTJSError(TJSSeekError);
}
//---------------------------------------------------------------------------
tjs_uint64 TVPReadI64LE(iTJSBinaryStream *stream)
{
#if TJS_HOST_IS_BIG_ENDIAN
	tjs_uint8 buffer[8];
	TVPReadBuffer(stream, buffer, 8);
	tjs_uint64 ret = 0;
...
#else
	tjs_uint64 temp;
	TVPReadBuffer(stream, &temp, 8);
	return temp;
#endif
}
//---------------------------------------------------------------------------
tjs_uint32 TVPReadI32LE(iTJSBinaryStream *stream)
{
#if TJS_HOST_IS_BIG_ENDIAN
	tjs_uint8 buffer[4];
	TVPReadBuffer(stream, buffer, 4);
	tjs_uint32 ret = 0;
...
#else
	tjs_uint32 temp;
	TVPReadBuffer(stream, &temp, 4);
	return temp;
#endif
}
//---------------------------------------------------------------------------
tjs_uint16 TVPReadI16LE(iTJSBinaryStream *stream)
{
#if TJS_HOST_IS_BIG_ENDIAN
	tjs_uint8 buffer[2];
	TVPReadBuffer(stream, buffer, 2);
	tjs_uint16 ret = 0;
