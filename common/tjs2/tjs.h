...
public:
	...
	bool LoadByteCode( class iTJSBinaryStream* stream, tTJSVariant *result = NULL,
		iTJSDispatch2 *context = NULL, const tjs_char *name = NULL);

	// for Binary Dictionay Array
	static bool LoadBinaryDictionayArray( class iTJSBinaryStream* stream, tTJSVariant *result );

	static bool LoadTextDictionaryArray( class iTJSTextReadStream* stream, tTJSVariant *result, const tjs_char *name = nullptr );

	void CompileScript( const tjs_char *script, class iTJSBinaryStream* output, bool isresultneeded = false, bool outputdebug = false, bool isexpression = false, const tjs_char *name = NULL, tjs_int lineofs = 0 );
};
//---------------------------------------------------------------------------
...
tjs_uint64 TVPReadI64LE(iTJSBinaryStream *stream); // reads little-endian integers
