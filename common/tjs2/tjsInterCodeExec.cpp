...
#define OBJ2 ((objthis)?(objthis):(Dispatch2))
		...
		tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *objthis)
	{
		...
		if(hr == TJS_E_MEMBERNOTFOUND && Dispatch1 != Dispatch2 && Dispatch2)
	...
	const tjs_int32 *code, tjs_uint32 flags)
{
	...
	if(ra_code3->Type() != tvtInteger)
	{
		...
		try
		{
			...
			hr = clo.PropGet(flags, str ? *str : TJS_W(""), NULL, TJS_GET_VM_REG_ADDR(ra, code[1]),
				clo.ObjThis?clo.ObjThis:ra[-1].AsObjectNoAddRef());
			if(TJS_FAILED(hr)) TJSThrowFrom_tjs_error(hr, str ? *str : TJS_W(""));
		}
		catch(...)
		{
			if(str) str->Release();
	...
	const tjs_int32 *code, tjs_uint32 flags)
{
	...
	if(ra_code2->Type() != tvtInteger)
	{
		...
		try
		{
			...
			if(hr == TJS_E_NOTIMPL)
				hr = clo.PropSet(flags,
					str ? *str : TJS_W(""), NULL, TJS_GET_VM_REG_ADDR(ra, code[3]),
						clo.ObjThis?clo.ObjThis:ra[-1].AsObjectNoAddRef());
			if(TJS_FAILED(hr)) TJSThrowFrom_tjs_error(hr, str ? *str : TJS_W(""));
		}
		catch(...)
	...
	const tjs_int32 *code, tjs_uint32 ope)
{
	...
	if(ra_code3->Type() != tvtInteger)
	{
		...
		try
		{
			hr = clo.Operation(ope, str ? *str : TJS_W(""), NULL,
				code[1]?TJS_GET_VM_REG_ADDR(ra, code[1]):NULL,
				TJS_GET_VM_REG_ADDR(ra, code[4]),
					clo.ObjThis?clo.ObjThis:ra[-1].AsObjectNoAddRef());
			if(TJS_FAILED(hr)) TJSThrowFrom_tjs_error(hr, str ? *str : TJS_W(""));
	...
	const tjs_int32 *code)
{
	...
	try
	{
		hr = clo.DeleteMember(0, str ? *str : TJS_W(""), NULL,
			clo.ObjThis?clo.ObjThis:ra[-1].AsObjectNoAddRef());
		if(code[1])
		{
			if(TJS_FAILED(hr))
	...
	const tjs_int32 *code, tjs_uint32 flags)
{
	...
	if(TJS_GET_VM_REG(ra, code[3]).Type() != tvtInteger)
	{
		...
		try
		{
			...
			else if(TJS_FAILED(hr)) TJSThrowFrom_tjs_error(hr, str ? *str : TJS_W(""));
		}
		catch(...)
		{
			if(str) str->Release();
