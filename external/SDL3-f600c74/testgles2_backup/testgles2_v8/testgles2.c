...
bool CommonInit(const char *window_title)
{
...
gl_profile_mask = SDL_GL_CONTEXT_PROFILE_ES;
        ...
        {
            SDL_GetClosestFullscreenDisplayMode(displayID, window_w, window_h, 0, false, &fullscreen_mode);
        }
...
//FIXME:
            if (!(window_flags & (SDL_WINDOW_OPENGL | SDL_WINDOW_VULKAN | SDL_WINDOW_METAL))) {
                ...
                logical_w = window_w;
                logical_h = window_h;
                if (!SDL_SetRenderLogicalPresentation(g_renderer, logical_w, logical_h, SDL_LOGICAL_PRESENTATION_DISABLED)) {
...
SDL_AppResult CommonEventMainCallbacks(const SDL_Event *event)
{
    switch (event->type) {
...
void CommonEvent(SDL_Event *event, int *done)
{
    if (CommonEventMainCallbacks(event) != SDL_APP_CONTINUE) {
        *done = 1;
    }
}
...
static void render_window()
{
    ...
    if (!SDL_GL_MakeCurrent(g_window, context)) {
