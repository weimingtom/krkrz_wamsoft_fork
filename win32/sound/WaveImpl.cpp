...
#include <InitGuid.h> // for DSDEVID_DefaultPlayback
#include <mmsystem.h>
#include <mmreg.h>
#include <math.h>
#include <algorithm>
...
static GUID TVPCurrentDirectSoundGUID = {0};
//---------------------------------------------------------------------------
...
static HRESULT (WINAPI *TVPDirectSoundEnumerateW)(LPDSENUMCALLBACKW, LPVOID) = NULL;
static HRESULT (WINAPI *TVPDirectSoundGetDeviceID)(LPCGUID, LPGUID) = NULL;
...
//---------------------------------------------------------------------------
static bool DSoundGetDriverVersion(ttstr &log, const wchar_t *strDrvName, const tjs_char *separator)
{
	...
	const wchar_t * strDrvName,  VOID* pContext )
{
	...
	if(strDrvName && strDrvName[0])
	{
		...
		if (!DSoundGetDriverVersion(log, strDrvName, TJS_W(" ")))
		{
			log += TJS_W(" ... is not found in search path");
		}
...
static void TVPInitDirectSound()
{
	...
	if(TVPDirectSoundDLL == NULL)
	{
		...
		try
		{
			...
			TVPDirectSoundEnumerateW = (HRESULT (WINAPI *)(LPDSENUMCALLBACKW, LPVOID)) GetProcAddress(TVPDirectSoundDLL, "DirectSoundEnumerateW");
			if(TVPDirectSoundEnumerateW)
				TVPDirectSoundEnumerateW(DSoundEnumCallback, NULL);
		}
		catch(...)
		{
			// ignore errors
		...
		// for GetDeviceList
		TVPDirectSoundGetDeviceID = (HRESULT (WINAPI *)(LPCGUID, LPGUID)) GetProcAddress(TVPDirectSoundDLL, "GetDeviceID");
	}
	...
	try
	{
		...
		HRESULT hr;
		ZeroMemory(&TVPCurrentDirectSoundGUID, sizeof(GUID));
		// create DirectSound with GUID
		{
			tTJSVariant val;
			if(TVPGetCommandLine(TJS_W("-wsguid"), &val))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					if(SUCCEEDED(hr))
					{
						TVPCurrentDirectSoundGUID = dsguid;
					}
					else
					{
						TVPAddImportantLog(TVPFormatMessage(TVPCannotInitDirectSound,
							ttstr(TJS_W("DirectSoundCreate failed./GUID=")) + ttstr(val) +
							ttstr(TJS_W("/HR=")) + TJSInt32ToHex((tjs_uint32)hr)));
					}
				}
			}
		}
	...
	catch(...)
	{
		...
		TVPDirectSoundEnumerateW = NULL;
		TVPDirectSoundGetDeviceID = NULL;
		throw;
	}
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
static tjs_int GetDeviceList(tTJSVariant &result)
{
	result.Clear();
	if (!TVPDirectSoundEnumerateW) return -1;

	GUID defaultpb = {0};
	const GUID *current = &TVPCurrentDirectSoundGUID;
	const bool iszero = !memcmp(current, &defaultpb, sizeof(GUID));
	const bool hasdefpb = TVPDirectSoundGetDeviceID && (DS_OK == TVPDirectSoundGetDeviceID(&DSDEVID_DefaultPlayback, &defaultpb));

	if (iszero)
	{
		if (hasdefpb) current = &defaultpb;
		else current = NULL;
	}
	...
	struct CallbackWork
	{
		...
		iTJSDispatch2 * array;
		const GUID * current;
		const GUID * defaultpb;

		CallbackWork(iTJSDispatch2 *array, const GUID *current, const GUID *defaultpb)
			: index(0), array(array), current(current), defaultpb(defaultpb) {}
		bool callback(GUID *guid, const ttstr &desc, const ttstr &module)
		{
			...
			try
			{
				...
				if (guid)
				{
					VariantFromGUID(val, *guid);
					item_dic->PropSet(TJS_MEMBERENSURE, TJS_W("guid"), NULL, &val, item_dic);

					val = (tjs_int)1;
					if (current && !memcmp(current, guid, sizeof(GUID)))
					{
						item_dic->PropSet(TJS_MEMBERENSURE, TJS_W("current"), NULL, &val, item_dic);
					}
					if (defaultpb && !memcmp(defaultpb, guid, sizeof(GUID)))
					{
						item_dic->PropSet(TJS_MEMBERENSURE, TJS_W("default"), NULL, &val, item_dic);
					}
				}

				val = desc;
				item_dic->PropSet(TJS_MEMBERENSURE, TJS_W("description"), NULL, &val, item_dic);
				...
				array->PropSetByNum(TJS_MEMBERENSURE, index++, &item_dic_var, array);
			}
			catch(...)
			{
				item_dic->Release();
		...
		static BOOL CALLBACK EnumCallback(GUID* pGUID, const wchar_t * strDesc, const wchar_t * strModule,  VOID* pContext)
		{
			CallbackWork *work = reinterpret_cast<CallbackWork*>(pContext);
			return work->callback(pGUID, ttstr(strDesc), ttstr(strModule)) ? TRUE : FALSE;
		}
	};
	...
	try
	{
		CallbackWork callback(array, current, hasdefpb ? &defaultpb : NULL);
		TVPDirectSoundEnumerateW(CallbackWork::EnumCallback, (LPVOID)&callback);
		ret = callback.index;
	}
	catch(...)
	{
...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/getDeviceList)  /* static */
{
	if(result) GetDeviceList(*result);

	return TJS_S_OK;
}
TJS_END_NATIVE_STATIC_METHOD_DECL_OUTER(cls, getDeviceList)
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(currentDevice)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		if (result)
		{
			GUID zero = {0};
			if (!memcmp(&TVPCurrentDirectSoundGUID, &zero, sizeof(GUID))) result->Clear();
			else VariantFromGUID(*result, TVPCurrentDirectSoundGUID);
		}
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
	...
	TJS_BEGIN_NATIVE_PROP_SETTER
	{
		TVPSetCommandLine(TJS_W("-wsguid"), ttstr(*param));
		TVPReleaseDirectSound();
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_SETTER
}
TJS_END_NATIVE_STATIC_PROP_DECL_OUTER(cls, currentDevice)
//----------------------------------------------------------------------
	return cls;
}
//---------------------------------------------------------------------------
