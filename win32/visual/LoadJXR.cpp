...
void TVPSaveAsJXR(void* formatdata, iTJSBinaryStream* dst, const class tTVPBaseBitmap* image, const ttstr & mode, class iTJSDispatch2* meta )
{
	CoInitialize(NULL);
	{
...
void TVPLoadHeaderJXR(void* formatdata, iTJSBinaryStream *src, iTJSDispatch2** dic )
{
	CoInitialize(NULL);
	{
		tTVPIStreamAdapter* s = new tTVPIStreamAdapter( src );
...
static Bool JXR_EOS(struct WMPStream* pWS) {
	iTJSBinaryStream* src = ((iTJSBinaryStream*)(pWS->state.pvObj));
