...
//---------------------------------------------------------------------------
iTJSBinaryStream * TJS_INTF_METHOD tTVPFileMedia::Open(const ttstr & name, tjs_uint32 flags)
{
...
// TVPOpenStream
//---------------------------------------------------------------------------
iTJSBinaryStream * TVPOpenStream(const ttstr & _name, tjs_uint32 flags)
{
	// open storage named "name".
	// currently only local/network(by OS) storage systems are supported.
	if(_name.IsEmpty())
...
// TTVPStreamAdapter
//---------------------------------------------------------------------------
__fastcall TTVPStreamAdapter::TTVPStreamAdapter(iTJSBinaryStream *ref)
{
	Stream = ref;
	TVPSetPosition(Stream, 0);
}
//---------------------------------------------------------------------------
...
*/
tTVPIStreamAdapter::tTVPIStreamAdapter(iTJSBinaryStream *ref)
{
	Stream = ref;
	RefCount = 1;
}
...
{
	// convert iTJSBinaryStream to IStream thru TStream
...
tTVPPluginHolder::tTVPPluginHolder(const ttstr &aname)
{
	...
	else
	{
		...
		pname = Application->PluginPath().c_str();
		pname += aname;
		if(TVPCheckExistentLocalFile(pname))
		{
			LocalName = pname;
